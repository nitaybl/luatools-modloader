...
def _download_zip_for_app(appid: int):
    ...
    if not apis:
        logger.warn(f"LuaTools: No enabled APIs found in manifest for app {appid}. Check your API configuration.")
        _set_download_state(appid, {"status": "failed", "error": "No manifest APIs available"})
        return
    ...
    for api in apis:
        ...
        try:
            ...
            import urllib.error
            error_type = "timeout" if isinstance(err, (TimeoutError, urllib.error.URLError)) and "timeout" in str(err).lower() else "error"
            error_code = None
            if hasattr(err, "status_code"):
                error_code = getattr(err, "status_code")
